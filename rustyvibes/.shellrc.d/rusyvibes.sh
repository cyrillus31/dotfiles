...
function rv {
  ...
  if [[ -z $1 ]]; then
    ...
    kill $(pgrep -f rustyvibes) &>/dev/null
    echo "$FOLDER"
    SOUNDPACK=$(find "$FOLDER" -mindepth 1 -maxdepth 1 -type d -exec basename {} \; | fzf --reverse --prompt 'Pick a soundpack> ' --header 'RUSTYVIBES Soundpacks: ')
    if [[ -z $SOUNDPACK ]]; then
...
EOF
    ...
    -s|--soundpack)
      ...
      SP=$(find "$FOLDER" | grep --max-count=1 "$2")
      (rustyvibes "$SP" &)
      ;;
    *)
